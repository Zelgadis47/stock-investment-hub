...
<li><b>同花顺i问财</b>：用自然语言查询"最新PMI数据"</li>
    ...
    this.addStrategy({
      name: '银行股轮动策略（年化34%）',
      logic: '在16只上市银行股中，按PE由低到高排序，选取前3只。要求PE>0且PB<1（破净股）。每月调仓一次，等权持有3只。卖出条件：PE排名掉出前5名。核心逻辑：银行股同质化高，低PE银行有估值修复动力。',
      condition: '银行板块整体PB<1时效果最佳（破净状态）。利率下行周期银行利差承压时应谨慎。',
      status: '跟踪中',
    });
    this.addStrategy({
      name: '北向资金跟踪策略',
      logic: '取前一个交易日北向资金净买入TOP10的股票，次日开盘买入前3只。持有3-5个交易日。卖出条件：持仓满5日或股价跌破5日均线。北向数据在前一交易日晚间通过港交所公布。',
      condition: '北向资金为持续净流入阶段（外资看好A股）。汇率稳定或人民币升值期效果更好。',
      status: '跟踪中',
    });
    this.addStrategy({
      name: '次新小盘策略',
      logic: '选股：上市60-365天，流通市值<30亿，PE>0且<80，排除ST和创业板。每周调仓一次，持有5只。买入条件：近20日涨幅<30%（避免追高）。止盈：盈利>20%卖一半，>30%全卖。止损：亏损>10%卖出。',
      condition: '熊市末期或震荡市中表现最佳。牛市该策略可能跑输大盘（资金集中于龙头）。',
      status: '跟踪中',
    });
    this.addStrategy({
      name: '均值回归（Z-Score）策略',
      logic: '筛选近60日波动率>20%的中证500成分股。计算60日Z-Score = (当前价 - MA60) / 标准差。当Z-Score < -2时买入（超卖），Z-Score > 1.5时卖出（回归均值）。适合趋势不明朗的震荡市场。',
      condition: '市场处于宽幅震荡或箱体整理阶段。单边牛市可能踏空，单边熊市可能抄底在半山腰。',
      status: '已废弃',
    });
    this.addStrategy({
      name: '申万行业轮动策略',
      logic: '统计过去20个交易日申万一级行业指数涨幅排名，选取涨幅最大的行业。在该行业中选取流通市值最大的3只股票。每月第一个交易日调仓，等权持有。核心逻辑：强者恒强惯性效应。',
      condition: '行业分化明显（结构性行情）时效果最佳。全行业普涨或普跌时表现一般。',
      status: '跟踪中',
    });
    this.addStrategy({
      name: '祖鲁法则小盘成长策略',
      logic: '选股条件：PE<25、净利润同比增长率>30%、PEG<1（市盈率/增长率）、总市值<100亿的小盘成长股。满仓持有评分最高的1-2只股票。止盈15%，止损8%。最高价回撤8%时提前卖出。',
      condition: '牛市或结构性行情中，成长风格占优时表现最佳。2014-2018年聚宽回测收益1465%',
      status: '已执行',
    });
...
<p><b>以下七种不做</b>：大盘不好不做 / 非主线不做 / 没有把握不做 / 没有卖出计划不买 / 破位不止损不行 / 心情不好不做 / 过了最佳位置不追。</p>`,
    ...
    // ===== 量化策略精粹（取自聚宽平台99份策略） =====
    this.addArticle({
      title: '聚宽平台策略精粹：12类量化策略核心逻辑解析',
      category: '其他',
      content: `<h3>概述</h3><p>以下策略均来源于聚宽（JoinQuant）量化平台用户分享的99份公开策略源码，经剔除重叠和低质策略后，提炼出12种独立逻辑。每类策略都包含核心选股条件、买卖逻辑和关键参数。</p>

<h4>1️⃣ 行业龙头精选</h4><p><b>来源</b>：#07 霍华罗斯曼价值精选、#33 买入每个行业市值最大</p><p><b>选股</b>：每个申万行业选取市值最大的1-3只龙头股。总市值≥50亿，PE>0且<30，ROE>15%，连续5年经营性现金流为正。<br><b>买卖</b>：每月调仓一次，等权配置5-10只。卖出条件：市值掉出行业前5名，或PE异常。<br><b>回测</b>：12年年化18%，最大回撤22%。</p>

<h4>2️⃣ 银行股轮动策略</h4><p><b>来源</b>：#64 12年年化34%银行股轮动</p><p><b>选股</b>：16只上市银行股中，按PE由低到高排序，选前3只。要求PE>0且PB<1（市净率破净）。<br><b>买卖</b>：每月调仓，等权持有。卖出条件：PE排名掉出前5名。<br><b>回测</b>：12年年化34%，Sharpe 1.2，最大回撤15.8%。</p>

<h4>3️⃣ 北向资金跟踪策略</h4><p><b>来源</b>：#38 跟踪聪明钱、#73 跟着港资买A股</p><p><b>选股</b>：取前一个交易日北向资金净买入TOP10的股票。过滤停牌股、ST股和创业板。<br><b>买卖</b>：次日开盘买入前3只，等权持有。持有3-5个交易日。卖出条件：持仓5日或股价跌破5日均线。<br><b>特点</b>：北向资金数据在交易日晚间公布，次日上午开盘操作。</p>

<h4>4️⃣ 低PB价值投资策略</h4><p><b>来源</b>：#31 低PB价值投资</p><p><b>选股</b>：沪深300成分股中，筛选PB<1.5且PE<15，股息率>2%，总市值>100亿的股票。<br><b>买卖</b>：每月调仓，等权配置10只。当PB>3或PE>30时卖出。<br><b>止损</b>：单只股票亏损>15%时卖出。</p>

<h4>5️⃣ 次新小盘策略</h4><p><b>来源</b>：#47 次新小盘、#14 中短期低市值</p><p><b>选股</b>：上市60-365天，流通市值<30亿，PE>0。排除ST股和创业板。<br><b>买卖</b>：每周调仓，持有5只。买入标准：近20日涨幅<30%（避免追高）。<br><b>止盈</b>：盈利>20%时卖出一半，>30%全卖。<br><b>特点</b>：次新股筹码干净，主力容易拉升。熊市该策略表现突出。</p>

<h4>6️⃣ 低估值TRIX+RSI择时</h4><p><b>来源</b>：#42 低估值+TRIX+RSI低回撤</p><p><b>选股</b>：沪深300成分股中，PE<20且PB<2，RSI(6日)<30（超卖）。<br><b>买卖</b>：TRIX指标上穿信号线时买入，持有至RSI>70或TRIX下穿时卖出。<br><b>止损</b>：亏损>8%无条件止损。<br><b>特点</b>：低回撤策略，适合震荡市。</p>

<h4>7️⃣ 均值回归策略（Z-Score）</h4><p><b>来源</b>：#12 均值回归(zscore)胜率100%</p><p><b>选股</b>：单只股票（如比亚迪）。计算60日Z-Score = (当前价 - MA60) / 标准差。<br><b>买卖</b>：Z-Score < -2时买入（超卖），Z-Score > 1时卖出（回归均值）。<br><b>仓位</b>：满仓操作。<br><b>特点</b>：适合趋势不明显的震荡市，单边牛市可能踏空。</p>

<h4>8️⃣ 资金流跟踪策略</h4><p><b>来源</b>：#95 资金流策略</p><p><b>选股</b>：全市场筛选当日主力资金净流入最大的50只股票。要求近3日资金累计净流入为正，且股价涨幅<5%（避免追涨）。<br><b>买卖</b>：每日开盘筛选，持有5只，等权配置。持仓5个交易日自动卖出。<br><b>止损</b>：股价跌破5日均线或资金转为大幅流出时提前卖出。</p>

<h4>9️⃣ 申万行业轮动策略</h4><p><b>来源</b>：#68 板块轮动、#81 申万行业轮动</p><p><b>选股</b>：选取过去20个交易日涨幅最大的申万一级行业指数。在该行业中选取流通市值最大的5只股票。<br><b>买卖</b>：每月第一个交易日调仓，等权持有。<br><b>回测</b>：年化收益15-25%，在行业分化明显的市场中表现最佳。</p>

<h4>🔟 祖鲁法则（Zulu Principle）</h4><p><b>来源</b>：#23 Zulu Principle A股实现</p><p><b>选股</b>：PE<20、净利润同比增长率>30%、PEG<1（市盈率/增长率）、总市值<100亿的小盘成长股。<br><b>买卖</b>：满仓持有最优评分股票。止盈15%，止损8%。<br><b>回测</b>：2014-2018年收益1465%，Sharpe 3.14，但空仓时间较多。</p>

<h4>1️⃣1️⃣ MACD金叉单因子</h4><p><b>来源</b>：#08 MACD单因子多头策略</p><p><b>选股</b>：沪深300成分股中，筛选日线MACD金叉（DIF上穿DEA）的股票。要求金叉发生在零轴上方（多头市场）。<br><b>买卖</b>：金叉次日买入，死叉卖出。持有不超过20个交易日。<br><b>止损</b>：MACD死叉或亏损>5%即卖出。</p>

<h4>1️⃣2️⃣ 自适应均线（AMA）期货策略</h4><p><b>来源</b>：#01 AMA自适应均线</p><p><b>选股</b>：适用于商品期货市场（RB螺纹钢、TAPTA、CU铜等10个品种）。使用AMA代替传统均线，在趋势和震荡中自适应调整速度。<br><b>买卖</b>：价格在AMA上方做多，下方做空。ATR止损倍数4倍。<br><b>特点</b>：自适应参数避免均线反复交叉的损耗。</p>

<h4>如何将这些策略融入你的系统</h4><p>打开<b>策略池</b> → 点击"新增策略"→ 将上述逻辑填入"核心逻辑"字段 → 保存后点击"🔍 匹配"实时筛选符合条件的股票。</p>`,
    });
